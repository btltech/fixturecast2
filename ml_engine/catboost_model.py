...
class CatBoostModel:
    ...
    def train(self, X, y):
        ...
        print(f"CatBoost training complete. Used {len(feature_keys)} numeric features.")
    ...
    def predict(self, features):
        ...
        home_xg = (home_attack / league_avg) * (away_defense / league_avg) * league_avg * 1.2  # Home boost
        ...
        home_cs_rate = features.get('home_clean_sheets', 5) / max(features.get('home_total_matches', 20), 1)
        away_cs_rate = features.get('away_clean_sheets', 5) / max(features.get('away_total_matches', 20), 1)
