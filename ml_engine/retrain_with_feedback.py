...
def generate_performance_report():
    ...
    for level in ['high', 'medium', 'low']:
        ...
        if stats.get('total', 0) > 0:
            print(f"   {level.upper()}: {stats.get('accuracy_pct', 'N/A')} "
    ...
    for model, stats in sorted_models:
        if stats.get('total', 0) >= 3:
            print(f"   {model}: {stats.get('accuracy_pct', 'N/A')} "
                  f"({stats.get('correct', 0)}/{stats.get('total', 0)})")
    ...
    for league_id, stats in sorted_leagues:
        if stats.get('total', 0) > 0:
            ...
            print(f"   {stats.get('name', league_id)}: {acc*100:.1f}% "
                  f"({stats.get('correct', 0)}/{stats.get('total', 0)})")
    ...
    for period, stats in sorted(recent.items()):
        if stats.get('total', 0) > 0:
            ...
            print(f"   {emoji} {period.replace('_', ' ').title()}: "
    ...
    for bin_key, stats in sorted(calibration.items()):
        if stats.get('count', 0) >= 3:
            ...
            print(f"   {bin_key}%: predicted {pred:.0f}% → actual {actual:.0f}% "
...
def main():
    ...
    parser.add_argument("--min-samples", type=int, default=20, 
                       help="Minimum samples for weight adjustment")
