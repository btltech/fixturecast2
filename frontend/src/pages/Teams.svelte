...
<div class="space-y-6 page-enter">
    ...
    <div class="flex justify-between items-center gap-3">
      ...
      <div class="league-selector relative">
        ...
        {#if showLeagueSelector}
          ...
          <div
            class="league-dropdown absolute right-0 top-full mt-2 w-64 max-h-80 overflow-y-auto bg-slate-900 border border-white/20 rounded-xl shadow-2xl z-50"
            on:click|stopPropagation
          >
            <!-- European -->
  ...
  {:else}
    <div
      class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4 element-enter stagger-1"
    >
      {#each filteredTeams as item}
        ...
        >
          ...
          />
          <span
            class="font-bold text-center text-sm md:text-base truncate w-full"
            >{item.team.name}</span
          >
          <span class="text-xs text-slate-500 truncate w-full text-center"
            >{item.venue.name}</span
          >
        </Link>
      {/each}
    </div>
