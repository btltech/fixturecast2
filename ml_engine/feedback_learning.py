...
class FeedbackLearningSystem:
    ...
    def __init__(self):
        ...
        self.model_performance = self._load_json(MODEL_PERFORMANCE_FILE, default={
    ...
    def log_prediction(self, 
                       ...
                       match_date: str,
                       prediction: Dict,
                       model_breakdown: Dict = None) -> Dict:
        """
        ...
        existing_idx = next(
            (i for i, p in enumerate(self.predictions_log) if p['fixture_id'] == fixture_id),
            None
        )
    ...
    def record_result(self,
        ...
        pred_entry = next(
            (p for p in self.predictions_log if p['fixture_id'] == fixture_id),
            None
        )
    ...
    def _update_performance_stats(self, pred_entry: Dict, evaluation: Dict):
        ...
        perf['calibration']['bins'][bin_key]['actual_sum'] += 1 if evaluation['outcome_correct'] else 0
...
def log_prediction(fixture_id: int, home_team: str, away_team: str, 
                   league_id: int, league_name: str, match_date: str,
                   prediction: Dict, model_breakdown: Dict = None) -> Dict:
    ...
    return feedback_system.log_prediction(
        ...
        match_date, prediction, model_breakdown
    )


def record_result(fixture_id: int, home_goals: int, away_goals: int, 
                  status: str = "FT") -> Optional[Dict]:
...
if __name__ == "__main__":
    ...
    print(f"Logged prediction: {logged['prediction']['predicted_outcome']} "
