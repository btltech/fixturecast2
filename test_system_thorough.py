...
def test_prediction_flow():
    ...
    try:
        ...
        print(
            f"Probabilities: Home {pred_data['home_win_prob']:.1%} | Draw {pred_data['draw_prob']:.1%} | Away {pred_data['away_win_prob']:.1%}"
        )
        print(
            f"Confidence: {pred_data['confidence_intervals'].get('confidence_level', 'N/A').upper()}"
        )
