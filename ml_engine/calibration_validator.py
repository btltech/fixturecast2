...
def find_optimal_temperature(predictions, outcomes, initial_temp=1.15):
    ...
    def apply_temperature(probs_list, T):
        ...
        for probs in probs_list:
            ...
            if total > 0:
                ...
                calibrated.append(
                    {"home_win_prob": 1 / 3, "draw_prob": 1 / 3, "away_win_prob": 1 / 3}
                )
    ...
    return {
        ...
        "message": (
            f"Optimal T={result.x:.3f} improves Brier score by {improvement_pct:.1f}%"
...
def validate_calibration(prediction_log_file):
    ...
    try:
        ...
        predictions = [
            item["prediction"] for item in data if "prediction" in item and "outcome" in item
        ]
