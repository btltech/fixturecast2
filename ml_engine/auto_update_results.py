...
def update_results_from_api(leagues: List[int] = None) -> Dict:
    ...
    return {
        "updated": updated,
        "pending": len(pending_ids) - updated,
        "errors": len(errors)
    }
...
def check_model_performance():
    ...
    for level in ['high', 'medium', 'low']:
        ...
        if stats.get('total', 0) > 0:
            print(f"  {level.capitalize()}: {stats.get('accuracy_pct', 'N/A')} "
    ...
    sorted_leagues = sorted(by_league.items(),
    ...
    if recommended:
        ...
        sorted_weights = sorted(recommended.items(), 
                               key=lambda x: x[1], 
                               reverse=True)
