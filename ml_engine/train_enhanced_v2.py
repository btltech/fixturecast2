...
def build_enhanced_features(matches, elo_tracker):
    ...
    for idx, match in enumerate(matches):
        ...
        # Calculate streaks
        home_win_streak = sum(
            1 for i, r in enumerate(home_form) if r == 3 and all(x == 3 for x in home_form[: i + 1])
        )
        away_win_streak = sum(
            1 for i, r in enumerate(away_form) if r == 3 and all(x == 3 for x in away_form[: i + 1])
        )
        ...
        home_momentum = sum((0.9**i) * r for i, r in enumerate(home_form)) / max(
            1, sum(0.9**i for i in range(len(home_form)))
        )
        away_momentum = sum((0.9**i) * r for i, r in enumerate(away_form)) / max(
            1, sum(0.9**i for i in range(len(away_form)))
        )
        ...
        features = {
            ...
            "home_form_last5": (
                sum(home_form[-5:]) if len(home_form) >= 5 else sum(home_form) if home_form else 7.5
            ),
            "away_form_last5": (
                sum(away_form[-5:]) if len(away_form) >= 5 else sum(away_form) if away_form else 7.5
            ),
            # Momentum (NEW)
            "home_momentum": home_momentum if home_form else 1.5,
            "away_momentum": away_momentum if away_form else 1.5,
            "momentum_diff": (home_momentum if home_form else 1.5)
            - (away_momentum if away_form else 1.5),
            # Streaks (NEW)
...
def train_with_cross_validation(X, y, n_splits=5):
    ...
    feature_keys = [
        k for k in X[0].keys() if k not in exclude_keys and isinstance(X[0].get(k), (int, float))
    ]
    ...
    cv_scores = {"accuracy": [], "brier": [], "log_loss": []}
    ...
    print(
        f"  CV Mean LogLoss: {np.mean(cv_scores['log_loss']):.4f} (±{np.std(cv_scores['log_loss']):.4f})"
    )
...
def train_meta_model(X, y, base_models, feature_keys):
    ...
    np.array([[sample.get(k, 0) for k in feature_keys] for sample in X])
    y_array = np.array(y)
    ...
    for i, sample in enumerate(X):
        ...
        if "elo" in base_models:
            elo_pred = base_models["elo"].predict_match(
                sample.get("home_id", 0), sample.get("away_id", 0)
            )
...
def main():
    ...
    if report["accuracy_by_confidence"]:
        print(
            f"   - High Confidence Accuracy: {report['accuracy_by_confidence'].get('high', {}).get('accuracy', 'N/A')}"
        )
        print(
            f"   - Medium Confidence Accuracy: {report['accuracy_by_confidence'].get('medium', {}).get('accuracy', 'N/A')}"
        )
